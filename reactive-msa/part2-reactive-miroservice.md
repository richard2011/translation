...
一个自治服务（[autonomous service](https://en.wikipedia.org/wiki/Service_autonomy_principle)）能够通过只保证（`promise`）[*注6*]拥有自己的行为，其他通过它的协议或API与其进行交互。拥抱这简单而又基本的事实，对我们如何理解与自治服务系统合作有深远的影响。
...
>在Unix哲学中，有一条古老的原则：所编写程序要只做一件事，并将其做好。 --- 道格.罗伊

Unix哲学[*注7*]和设计已经非常成功，提出几十年后仍然反响热烈。它其中一个关键原则就是开发者应该编写程序只有一项目标、很小但是具备定义清晰的责任并且能够很容易地与其他小程序进行组合。

这个理念后来纳入Robert C.Martin面向对象编程并称之为单一职责原则（SRP）[*注8*]，这个声明一个类或者组件应该是『只有一个原因会导致改变』。

已经有很多讨论的一个微服务的正真规模。什么称之为『微』？多少行代码才能称为微服务？这些都是错误的答案。『微』应该指的是职责的范围，Unix的哲学单一职责可以作为指导原则：只做一件事，并把它做好。

如果服务只会因为单个原因存在，提供一个可组合的功能，然后业务模型和职责不纠缠。每个服务都可以更广泛使用，系统作为一个整体更容易水平扩展、高可用、容易理解、容易扩展和维护。
...
到目前为止，我们把微服务作为一系列隔离服务，每个都有单独一项职责。这种形式可以把服务作为一个单独的单元，它们的生命周期都被隔离（高可用的先决条件），移动可以隔离（可扩展性的先决条件）。
...
我们需要每个微服务都负责自己的状态和持久化。构建每个服务就像一个边界上下文[*注9*]能够帮助每个服务都定义它自己的域，每个都有自己的通用语言。这些技术都来自领域驱动设计（[Domain-Driven Design，DDD](https://en.wikipedia.org/wiki/Domain-driven_design)）[*注10*]模型构建工具。这里介绍的所有新概念,考虑DDD学习一个很好的起点。DDD对微服务有着深远的影响，许多你听到的在微服务上下文词语都来源于DDD。

当你与其他微服器通信应该通过限界上下文，你只友好地请求一下它的状态 --- 你不能强迫暴露它。每个服务它会自己响应请求，从它的当前状态返回不可变数据（事实），同时从来不会直接暴露它的可变状态。

这样，每个服务自由地用什么方式展示它的状态，使用合适的规范和半规范化地存储。有些服务可能会选择一个在传统的关系数据库管理系统（[RDBMS](https://en.wikipedia.org/wiki/Relational_database_management_system)），如Oracle、Mysql和Postgres。有些是[NosSQL]()数据库（如[Cassandra]()和[Riak]()），有些是时序（[Time-Series]()）数据库（如[InfluxDB]()和[OpenTSDB]()），有些使用Event Log[*注11*]（很好的后端支持包括[Kafka]()，[Amazon Kinesis]()和Cassandra），来实现Event Souring[*注12*] 和 命令与查询职责分离（CQRS）。

分散式的数据管理和持久化有很多好处 --- 有时候称为*混合我持久化*。从概念上，用哪个存储中间件都重要，最重要的是服务能够看作是一个独立的单元（包括状态和行为）--- 为了每个服务都有自己的状态，非共享的。它包括一个服务不能直接访问另一个服务的持久化存储，只能通过API --- 有时候很给程序自动强制的，因此需要通过会议、规则和代码审核来完成。

*事件溯源（Event Log）* 是消息的持久化存储。我们现样可以存储从外面进入服务的消息，服务的命令，一般称之为*命令溯源*。我们同样可以选择忽略命令，让它执行它的服务side-effect，发果side effect触发一个服务的状态改变，我们能够捕捉这状态改变看作事件里一个新的事实，这个会存储在事件日志里，这时使用事件溯源。

消息被存储成有序，提供服务之间交互的全部历史，因为消息大多数据时候都代表服务的事务，事件日志本质上给我们提供了一个事务日志，这个日志能够显示地查询 、审计、弹性发布时用于回放消息（从任意的时间点）、调试和复制 -- 而不像RDBMS那样地需要使用者抽象出来，Pat Helland 充分证明这点：

> 事务日志记录着数据库的所有改变，只能通过高性能地添加来改变日志。从这个观点来看，数据库的内容只是维护日志里的最新记录内容的缓存。日志才是真相。数据库日志子集的一个缓存 。这缓存子集是每条记录的最后的值和日志的索引。[*注13*]

命令溯源和事件溯源有着不同的语义。例如，回放命令意味着你同样重放着它们代表的side effects；重放事件只是执行状态改变操作，加速服务的状态的变化。根据不同的用户场景来选择最合适的技术。
...
最说用一个例子来说明。如果一个服务调用另外一个服务产生了阻塞（等待结果返回）--- 它会占用线程。这意味着在这个执行周期内线程不能做任何事情。线程是稀有资源，需要尽可能地有效使用。如果服务使用异步和非阻碍模式调用，它就可以释放当用线程，然后做其他事件直到结果返回。它会更加有效地使用（减小线程成本、提高性能），图2-7

特别要指出服务限界的不同资源之间通信通过内嵌异步通信是非常重要的。为了充分利用非阻塞优势，所有的请求链都需要参与 -- 来自请求分发，通过服务实现，下到数据库和后端。

异步消息传递帮助我们产生约束（尤其是失败的场景，网络编程的第一课），而不是隐瞒在有漏洞的空想[*注14*]和假装它们不存在（就像同步[PRC]()的谬论[*注15*]）。
...
大数据第一阶段是『数据是静止』。我们存储大量记录到HDFS或者相似的系统，它们需要一个夜晚，通常是几个小时廷时来执行离线批量处理。

第二阶段，我们意到需要近实时地响应数据『数据是流动』 --- 捕获流动的数据，处理它然后在秒级甚至在次秒级内把结果返回到运行的系统（这个越来越重要）。

这需要使用混合的架构，就像[Lambda架构](http://lambda-architecture.net)，有两层含意：用于实时在线处理的『快速层』和用于完全离线处理的『批量层』；这就是从『快速层』实时处理然后合并到『批量层』。这个模型解决一些实时需要快速反应（最小子集）的数据。但是需要增加维护两个独立模型和数据处理管道的复杂性，同样数据需要在最后合并。
...
随着云计算、虚拟化和Docker容器，我们有很多手段来有效地管理硬件资源。问题是,如果我们微服务及其底层平台不能有效用，一切都没有意义。所以我们的服务需要可移动性，允许他们能够变得弹性。

我们已经谈论了异步信息传递，然而它提供解耦时间和空间。后者，空间解耦，称之为*位置透明*[*注19*]，微服务能够在运行时，动态扩展（无论是多核或多节点，不需要改变代码）。这就是分布式服务能够可弹性和移动性；它需要允分利用云计算和现收现付制的模式。
