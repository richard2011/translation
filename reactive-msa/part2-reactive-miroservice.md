...
命令溯源和事件溯源有着不同的语义。例如，回放命令意味着你同样回放着它们代表的side effects；回放事件只是执行状态改变操作，加速服务的状态的变化。根据不同的用户场景来选择最合适的技术。

使用事件日志同样能够避免对象-关系阻抗失配（[Object-Relational Impedance Mismatch]()），一个发生在[Object-Relational Mapping, ORM]()技术的问题。还可以建立消息传递通信的基础，然而这已经是主要通信机制的事实。使用事件日志同样也是微服务最后的存储模型，因为它天生支持异步消息传递通信（请看图2-4）
